...
export class AdminUsersService {
  ...
  async listUsers(params: {
    ...
    const now = new Date();
    const allGroupIds = items.flatMap((u) =>
      u.groupMembers.map((gm) => gm.groupId)
    );
    ...
    const itemsWithTripPassCount = items.map((u) => {
      const activeTripPasses = u.groupMembers.filter((gm) =>
        groupsWithTripPass.has(gm.groupId)
      ).length;
  ...
  async extendEntitlement(
    ...
    const newEndsAt = new Date(
      Math.max(before.endsAt.getTime(), Date.now()) +
